...
var acl = {
    ...
    updateACL: function(id, ui) {
        ...
        if (type === "public") {
            ...
            var myAudience = [];
            var updateMySelection = function(aclObj){
                var text = "You have selected ";
                var gl = aclObj.accept.groups.length
                var ul = aclObj.accept.users.length
                var fr = aclObj.accept.friends
                var isPlural = function(x){
                    return x==1?"":"s"
                }

                if (gl == 0 && ul == 0 && fr == false){
                    $("#custom-audience-dlg #footer-info").text("")
                    return
                }
                text += gl < 1?"":gl+" group"+isPlural(gl)
                if (gl>0 && (ul>0 || fr == true)){
                    text += " and "
                }
                text += fr == false? ul < 1?"":" "+ ul+" user"+isPlural(ul):" all your friends"
                $("#custom-audience-dlg #footer-info").text(text)

            }
            var removeItem = function(aclObjId, id, type, aclObj){
                var aclObjArray = type=="user"?aclObj.accept.users:aclObj.accept.groups
                var idx = jQuery.inArray(id, aclObjArray)
                if (idx != -1){
                    aclObjArray.splice(idx, 1)
                }
                $("#"+aclObjId).attr("value", $$.json.stringify(aclObj));
                updateMySelection(aclObj)
                //$("#custom-audience-dlg"+"-label").text(str);
            }
            aclObj.accept.groups = [];
            aclObj.accept.users = [];
            aclObj.accept.friends = false;
            $$.dialog.create({
                id: 'custom-audience-dlg'
            });
            var dlgXhr = $.get('/ajax/feed/audience');
            dlgXhr.then(function(){
                $("#allfriends").change(function(){
                    aclObj.accept.friends = this.checked
                    $("#"+id).attr("value", $$.json.stringify(aclObj));
                    updateMySelection(aclObj)
                })
                $("#custom-audience-dlg-search").autocomplete({
                    source: function(request, response ){
                        $.ajax({
                            url: '/auto/mycollection',
                            dataType: "json",
                            data: request,
                            success: function(data) {
                                var filteredData = [];
                                $.each(data, function(index, value) {
                                  if (jQuery.inArray(value.value, myAudience) == -1){
                    ...
                    minLength: 1,
                    select: function(event, obj){
                        myAudience.push(obj.item.value);
                        if (obj.item.type == "user"){
                            aclObj.accept.users.push(obj.item.value)
                        }else{
                            aclObj.accept.groups.push(obj.item.value)
                        }
                        var item = $(obj.item.label).prependTo($("#custom-audience-dlg .ui-dlg-center"));
                        var action = $(item).children(":first").siblings(":last")
                        var actionButton = $("<span class='conv-other-actions'>&nbsp;</span>")
                        actionButton.click(function(){
                            item.remove();
                            var idx = jQuery.inArray(obj.item.value, myAudience);
                            if ( idx != -1){
                                myAudience.splice(idx, 1);
                            }
                            removeItem(id, obj.item.value, obj.item.type, aclObj);
                        })
