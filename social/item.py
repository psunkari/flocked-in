...
class Item(object):
    ...
    def _notify(cls, notifyType, convId, timeUUID, **kwargs):
        deferreds = []
    ...
    @classmethod
    @defer.inlineCallbacks
    def like(cls, itemId, item, myId, orgId, me=None):
        try:
        ...
        if me.id != item["meta"]["owner"]:
            ...
            if itemId != convId:
                if me.id != itemOwnerId:
                    yield cls._notify("LC", convId, timeUUID, convType=convType,
                                  convOwnerId=convOwnerId, myId=me.id,
                                  itemOwnerId=itemOwnerId, me=me)
            else:
                if me.id != convOwnerId:
                    yield cls._notify("L", convId, timeUUID, convType=convType,
                                  convOwnerId=convOwnerId, myId=me.id, me=me)
        defer.returnValue(item)

    @classmethod
    @defer.inlineCallbacks
    def unlike(cls, itemId, item, myId, orgId):
    ...
    @classmethod
    @defer.inlineCallbacks
    def _comment(cls, convId, conv, comment, snippet, myId, orgId, richText, review):
        ...
        yield cls._notify("C", convId, timeUUID, convType=convType,
    ...
    @classmethod
    @defer.inlineCallbacks
    def tag(cls, itemId, item, tagName, myId, orgId):
    ...
    @classmethod
    @defer.inlineCallbacks
    def untag(cls, itemId, item, tagId, tag, myId, orgId):
        if "parent" in item:
    ...
    @classmethod
    @defer.inlineCallbacks
    def removeFromFeed(cls, itemId, item, myId, orgId):
    ...
    @classmethod
    @defer.inlineCallbacks
    def delete(cls, itemId, item, myId, orgId):
    ...
    @classmethod
    @defer.inlineCallbacks
    def new(cls, request, authInfo, convType, richText=False):
    ...
    @classmethod
    def _cleanupMissingComments(cls, convId, missingIds, itemResponses):
        missingKeys = []
    ...
    @classmethod
    @defer.inlineCallbacks
    def responses(cls, myId, itemId, item, start=''):
        toFetchEntities = set()
        ...
        if missingIds:
            yield Item._cleanupMissingComments(itemId, missingIds, itemResponses)
...
class ItemResource(base.BaseResource):
    ...
    def _unlike(self, request, data=None):
        ...
        if itemId != convId:
            ...
            if toFetchEntities:
                entities = base.EntitySet(toFetchEntities)
                entities = yield entities.fetchData()
    ...
    def _submitReport(self, request, action):
        ...
        if myId == convOwnerId:
            ...
            if action == "accept":
                ...
                yield deleteItem(convId, myId, orgId)
...
class APIItemResource(base.APIBaseResource):
    ...
    def _newItem(self, request):
        ...
        #TODO: call Item.new instead of _createNewItem
