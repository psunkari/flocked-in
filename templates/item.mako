...
<%def name="render_item_report_comment_form(reportedBy)">
  <div class="comment-form-wrapper busy-indicator" id="report-form-wrapper" >
    <form method="post" action="/item/report" class="ajax" autocomplete="off"
...
<%def name="render_item_report_actions(reportedBy)">
<ul class="middle user-actions h-links" id="report-actions">
  %if myKey != ownerId :
    %if reportedBy:
      ...
      %endif
    %else:
      <button onclick="$('#conv-report-action').attr('value', 'report')"
...
<%def name="render_report_comments()">
    ...
    <%
      if myKey == ownerId:
        ...
        if "reportId" not in convMeta:
          ...
          elif item-status == "re-edit":
            status = "%s has made changes to the original post. \
                      If you are satisfied with the changes, you may \
                      withdraw the complaint or reject it." %owned_by
    %>
    <span>${status}</span>
  </div>
</%def>
